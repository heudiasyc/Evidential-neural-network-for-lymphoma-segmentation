...
@alias("Unet_rbf_kmeans")

#####RBF for two class
class RBF_KMEANS(nn.Module):
    def __init__(self, input_dim, class_dim, prototype_dim, prototype):
        super(RBF_KMEANS, self).__init__()
...
class UNet_RBF_KMEANS(nn.Module):
    def __init__(
        self,
        dimensions: int,
        in_channels: int,
...
prototype=W_p.to(torch.float32)

Unet_rbf_kmeans = unet_rbf_kmeans = UNet_RBF_KMEANS
